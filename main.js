...
async function pdfToWord() {
  ...
  try {
    ...
    // Start task
    const response = await fetch("http://192.168.1.102:8000/api/convert/pdf-to-word", {
        method: "POST",
        body: formData
    });
    ...
    while (status === "pending" || status === "processing") {
        ...
        const statusRes = await fetch(`http://192.168.1.102:8000/api/status/${task_id}`);
    ...
    // Download
    const dlRes = await fetch(`http://192.168.1.102:8000${download_url}`);
